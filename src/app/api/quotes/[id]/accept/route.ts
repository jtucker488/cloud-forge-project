...
export async function POST(
  ...
  try {
    ...
    if (isNaN(quoteId)) {
      console.log('[ACCEPT] Invalid quoteId parameter');
    ...
    if (quoteError || !quote) {
      console.log('[ACCEPT] Failed to update quote status:', quoteError?.message);
      return NextResponse.json({ error: quoteError?.message || 'Failed to update quote status' }, { status: 500 });
    }
    ...
    if (lineItemsError) {
      console.log('[ACCEPT] Failed to fetch quote line items:', lineItemsError.message);
    ...
    if (salesOrderError || !salesOrder) {
      console.log('[ACCEPT] Failed to create sales order:', salesOrderError?.message);
    ...
    if (shipmentError || !shipment) {
      console.log('[ACCEPT] Failed to create shipment:', shipmentError?.message);
    ...
    for (const item of lineItems) {
      ...
      if (item.dimensions) {
        const match = item.dimensions.match(/L:\s*(\d+\.?\d*),\s*W:\s*(\d+\.?\d*),\s*T:\s*(\d+\.?\d*)/);
        if (match) {
      ...
      if (gradeLookupError || !gradeRow) {
        console.log('[ACCEPT] Grade not found for material_id', item.material_id, 'grade', item.grade);
      ...
      let inventoryQuery = supabase
        ...
        .eq('user_id', userId);
      if (length !== undefined) inventoryQuery = inventoryQuery.eq('length', length);
      if (width !== undefined) inventoryQuery = inventoryQuery.eq('width', width);
      if (thickness !== undefined) inventoryQuery = inventoryQuery.eq('thickness', thickness);
      ...
      if (inventoryError || !inventory) {
        console.log('[ACCEPT] Inventory not found for material_id', item.material_id, 'grade_id', grade_id, 'user_id', userId, 'length', length, 'width', width, 'thickness', thickness);
        return NextResponse.json({ error: `Inventory not found for material_id ${item.material_id}, grade_id ${grade_id}, user_id ${userId}, length ${length}, width ${width}, thickness ${thickness}` }, { status: 500 });
      }
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          allocated_quantity: (inventory.allocated_quantity || 0) + item.quantity,
        })
        .eq('id', inventory.id);
      console.log('[ACCEPT] inventory update result:', updateError);
      if (updateError) {
        console.log('[ACCEPT] Failed to update inventory for material_id', item.material_id, 'grade_id', grade_id, 'user_id', userId, 'length', length, 'width', width, 'thickness', thickness, updateError.message);
        return NextResponse.json({ error: `Failed to update inventory for material_id ${item.material_id}, grade_id ${grade_id}, user_id ${userId}, length ${length}, width ${width}, thickness ${thickness}` }, { status: 500 });
      }
    }

    console.log('[ACCEPT] Success!');
    return NextResponse.json({
      message: 'Quote accepted, sales order and shipment created, inventory adjusted',
      salesOrder,
      shipment,
    });
  } catch (err) {
